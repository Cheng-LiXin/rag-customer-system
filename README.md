...
- **拒答与低置信**：检索后、调模型**之前**判定最高相似度（`rag.answer.reject-threshold`，默认 **0.59**）——低于则直接拒答并引导转人工，**不调用大模型**（省时延、从根上杜绝幻觉）；落在 `[reject-threshold, low-confidence-threshold)` 则照常作答但标记低置信。阈值不是拍脑袋：150 题 Golden Set 实测，常规题相似度下限 0.5959、超纲题上限 0.6626 —— **两组存在重叠、不存在干净切点**，取值必然是一笔权衡。文档建议的 0.65 会误拒约 10.8% 正常题；0.59 在实测中取到「正常题误拒 0% + 超纲拒答 96.67%」。阈值可用 `tools/eval/threshold_sweep.py` 离线重扫（不必改配置重启）。详见 `docs/评估报告.md`。
